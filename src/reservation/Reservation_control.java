...
public String getConReservation(Reservation_view frame){
	String res ="";
	MySQL mysql = new MySQL();
	
	if(flagLogin){
		try {
			res = reservation_userid + "の予約状況\n";
			ResultSet rs = mysql.getConReservation(reservation_userid);
			boolean exist = false;
			while (rs.next()) {
...
public String getExp(Reservation_view frame){
	String res = "";
	MySQL mysql = new MySQL();
	
	 try {
		ResultSet rs;
		rs = mysql.getExp(frame.choiceFacility.getSelectedItem());
		while (rs.next()) {
...
/*public String getCan(Reservation_view frame){
	String res = "";
	MySQL mysql = new MySQL();
	try {
		ResultSet rs
		rs = mysql.getExp(frame.choiceCancel.getSelectedItem());
		while (rs.next()) {
			String fe = rs.getString("explanation");
			res = fe;
		} 
